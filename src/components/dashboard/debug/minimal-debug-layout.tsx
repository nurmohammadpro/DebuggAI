...
interface MinimalDebugLayoutProps {
  children: ReactNode;
}

export function MinimalDebugLayout({ children }: MinimalDebugLayoutProps) {
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] flex">
      ...
      <aside className="w-[280px] border-r border-[var(--border-default)] bg-[var(--bg-secondary)] flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto py-3">
          ...
          />
          <NavItem
            active={pathname === '/dashboard/debug' || pathname === '/dashboard/debug/history'}
            icon="🐛"
            label="Debug Session"
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        {/* Header */}
        <div className="h-14 border-b border-[var(--border-default)] flex items-center justify-between px-4 bg-[var(--bg-secondary)]">
          <div className="flex items-center gap-2">
            <span className="text-[12px] text-[var(--text-secondary)]">
              Debug Session <span className="text-[var(--text-tertiary)]">/</span> New Session
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Link
